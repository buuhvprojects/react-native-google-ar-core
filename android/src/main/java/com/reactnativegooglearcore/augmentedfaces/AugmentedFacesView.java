...
import com.reactnativegooglearcore.common.helpers.FullScreenHelper;
 ...
 */
public class AugmentedFacesView extends View implements GLSurfaceView.Renderer {
  private static final String TAG = AugmentedFacesView.class.getSimpleName();
  ...
  public AugmentedFacesView(@NonNull ThemedReactContext context) {
    super(context);
    this.context = context;
    trackingStateHelper = new TrackingStateHelper(this.context.getCurrentActivity());
    context.getCurrentActivity().setContentView(R.layout.activity_main);
    surfaceView = context.getCurrentActivity().findViewById(R.id.surfaceview);
    displayRotationHelper = new DisplayRotationHelper(/*context=*/ this.context);
  ...
  protected void start() {
    if (session == null) {
      ...
      try {
        ...
        if (!CameraPermissionHelper.hasCameraPermission(this.context.getCurrentActivity())) {
          CameraPermissionHelper.requestCameraPermission(this.context.getCurrentActivity());
          return;
        }

        // Create the session and configure it to use a front-facing (selfie) camera.
        session = new Session(/* context= */ this.context, EnumSet.noneOf(Session.Feature.class));
      ...
      if (message != null) {
        messageSnackbarHelper.showError(this.context.getCurrentActivity(), message);
    ...
    try {
      ...
      messageSnackbarHelper.showError(this.context.getCurrentActivity(), "Camera not available. Try restarting the app.");
      session = null;
      return;
    }
  ...
  public void onWindowFocusChanged(boolean hasFocus) {
    ...
    FullScreenHelper.setFullScreenOnWindowFocusChanged(this.context.getCurrentActivity(), hasFocus);
  }
  ...
  public void onSurfaceCreated(GL10 gl, EGLConfig config) {
    ...
    try {
      ...
      augmentedFaceRenderer.createOnGlThread(this.context, "models/freckles.png");
      augmentedFaceRenderer.setMaterialProperties(0.0f, 1.0f, 0.1f, 6.0f);
      noseObject.createOnGlThread(/*context=*/ this.context, "models/nose.obj", "models/nose_fur.png");
      noseObject.setMaterialProperties(0.0f, 1.0f, 0.1f, 6.0f);
      noseObject.setBlendMode(ObjectRenderer.BlendMode.AlphaBlending);
      rightEarObject.createOnGlThread(this.context, "models/forehead_right.obj", "models/ear_fur.png");
      ...
      leftEarObject.createOnGlThread(this.context, "models/forehead_left.obj", "models/ear_fur.png");
